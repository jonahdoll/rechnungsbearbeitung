...
public record Rechnungsmetadaten(
    @NotNull UUID id,
    @NotBlank String rechnungsnummer,
    @NotNull LocalDateTime rechnungsdatum,
    LocalDateTime faelligkeitsdatum,
    @NotBlank String rechnungsausteller,
    @NotBlank String rechnungsempfaenger,
    @NotEmpty @Valid List<Rechnungsposition> positionen,
    @NotBlank @Pattern(regexp = "^[A-Z]{2}[0-9]{2}[A-Z0-9]{11,30}$") String iban,
    @NotBlank @Size(max = 11) String bic) {
