...
public class ClientApplication {
    ...
    public static void main(String[] args) {
        try (var executor = Executors.newVirtualThreadPerTaskExecutor();
            ...
            Rechnungen rechnungen = xmlMapper.readValue(
                    ClientApplication.class.getResourceAsStream("/rechnungen.xml"),
                    Rechnungen.class
            );

            rechnungen.getRechnungen().forEach(rechnung -> executor.submit(() -> verarbeite(client, rechnung)));
    ...
    private static void sendeZahlungsauftrag(Rechnung r) throws Exception {
        try (var producer = new ZahlungsProducer()) {
            producer.sendeZahlungsauftrag(new Zahlungsauftrag(
                    r.getRechnungsnummer(), r.getGesamtBetrag(), r.getIban(), LocalDateTime.now().plusDays(30)
            ));
        }
    }
}
